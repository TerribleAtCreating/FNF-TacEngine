...
class StrumNote extends FlxSprite
{
	...
	public function new(x:Float, y:Float, leData:Int, player:Int) {
		...
		{
			loadGraphic(Paths.image('pixelUI/' + skin[player]));
			width = width / 4;
			height = height / 5;
			loadGraphic(Paths.image('pixelUI/' + skin[player]), true, Math.floor(width), Math.floor(height));
		...
		{
			frames = Paths.getSparrowAtlas(skin[player]);
